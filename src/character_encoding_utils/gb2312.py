...
class GB2312EncodeError(GB2312Exception):
    ...
    def __str__(self) -> str:
        return f"'gb2312' codec can't encode character '\\u{ord(self.obj):x}' in position {self.position}: {self.reason}"
...
class GB2312DecodeError(GB2312Exception):
    ...
    def __str__(self) -> str:
        if len(self.obj) <= 1:
            return f"'gb2312' codec can't decode byte 0x{self.obj[0]:x} in position {self.position}: {self.reason}"
        else:
            return f"'gb2312' codec can't decode bytes in position {self.position}-{self.position + len(self.obj) - 1}: {self.reason}"
