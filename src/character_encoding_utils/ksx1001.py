...
class KSX1001EncodeError(KSX1001Exception):
    ...
    def __str__(self) -> str:
        return f"'ksx1001' codec can't encode character '\\u{ord(self.obj):x}' in position {self.position}: {self.reason}"
...
class KSX1001DecodeError(KSX1001Exception):
    ...
    def __str__(self) -> str:
        if len(self.obj) <= 1:
            return f"'ksx1001' codec can't decode byte 0x{self.obj[0]:x} in position {self.position}: {self.reason}"
        else:
            return f"'ksx1001' codec can't decode bytes in position {self.position}-{self.position + len(self.obj) - 1}: {self.reason}"
